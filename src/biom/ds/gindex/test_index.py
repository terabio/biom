...
from biom.primitives import Direction, Range
...
def test_merge(empty_index, simple_index, complex_index):
    ...
    assert len(merged.itrees[('1', Direction.fwd)]) == 3
    assert len(merged.itrees[('2', Direction.fwd)]) == 3
    assert len(merged.itrees[('2', Direction.rev)]) == 2

    overlap = merged.overlap('1', Direction.fwd, 0, 10)
    assert overlap.intervals == [Range(0, 10), Range(1, 10)]
    assert overlap.annotations == ['-100:100', '1:10']

    overlap = merged.overlap('2', Direction.fwd, -3, 10)
    assert overlap.intervals == [Range(-3, 2), Range(0, 10)]
    assert overlap.annotations == ['-5:2', '0:150']

    overlap = merged.overlap('2', Direction.rev, -1, 8)
...
def test_overlap_both_range_and_start_end_provided(empty_index):
    with pytest.raises(ValueError):
        empty_index.overlap('1', Direction.fwd, 0, 10, Range(0, 10))


def test_overlap_no_range_provided(empty_index):
    with pytest.raises(ValueError):
        empty_index.overlap('1', Direction.fwd)


def test_overlap_no_hits(complex_index):
    overlap = complex_index.overlap('1', Direction.fwd, 100, 200)
...
def test_from_bed_with_filter(bed_file):
    ...
    overlap = index.overlap('chr1', Direction.fwd, 940, 10000)
    assert overlap.intervals == [Range(940, 1000)]
    assert overlap.annotations == ['gene1']

    for contig, strand in ('chr1', Direction.rev), ('chr2', Direction.fwd), ('chr2', Direction.rev):
